...
namespace Embarques
{
    public partial class ConsDetEmbdet : Form
    {
        ...
        public ConsDetEmbdet(string pedido)
        {
        ...
        private void ConsDetEmb_Load(object sender, EventArgs e)
        {
            ...
            thisConnecion.Open();
            string Mped = pedidooriginal.ToString();
        ...
        private void DetEti()
        {
            ...
            foreach (DataRow row in DetLotes.Select("prod_clave = '" + Mprod + "'"))
            {
                if (mREC != row["No_lote"].ToString().Substring(0, 6))
                {
                    ...
                    Mpe = (Fn_PesoXRecibo(mREC, Mprod, row["tipo_rec"].ToString(), Convert.ToInt32(row["CAJAS"])))/ Convert.ToInt32(row["CAJAS"]);
                }
                ...
                DGLotes.Rows.Add(Mlote, row["CAJAS"].ToString(), row["TEMP"].ToString(), Convert.ToInt16(row["SECCION"]), row["FEC_CAD"].ToString(), (Mpe * Convert.ToDecimal(row["CAJAS"])).ToString("##,##0.00"), Mpe.ToString("##0.00"), row["tipo_rec"].ToString());
            }
        ...
        {
            BusTipo(CmbPed.Text.ToString());
            LLenaPed(CmbPed.Text.ToString());
            FormatoSalida();
        }
        ...
        public decimal Fn_PesoXRecibo (string mRecibo, string Mprod, string TipoRec, int MCant)
        {
            decimal mKilos = 0;
            string Cadena = "";
            SqlCommand cmd;
            ...
            else
            {
                ...
                {                   
                    ...
                    mKilos = mKilos + ((Convert.ToDecimal(Info["HRP_PESO_NETO"]) / Muni) * MCant) + (Mpe * MCant); // +Mpe;
